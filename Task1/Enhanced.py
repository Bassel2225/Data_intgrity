...
class User(db.Model):
    ...
    secret = db.Column(db.String(32), nullable=True)  # Store 2FA secret
...
def generate_2fa():
    ...
    uri = pyotp.TOTP(user.secret).provisioning_uri(name=username, issuer_name='Data_Integrity_Section_2FA')
    qr = qrcode.make(uri)
    img = io.BytesIO()
    qr.save(img)
    img.seek(0)
